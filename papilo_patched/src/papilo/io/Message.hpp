...
// Helper to convert enums to their underlying type for fmt 10.x compatibility
// fmt 10.x requires explicit formatter specializations for enums
namespace detail {
    template<typename T>
    constexpr auto format_arg(T&& arg) {
        if constexpr (std::is_enum_v<std::remove_cv_t<std::remove_reference_t<T>>>) {
            return static_cast<std::underlying_type_t<std::remove_cv_t<std::remove_reference_t<T>>>>(arg);
        } else {
...
class Message
{
 ...
 public:
          ...
          Args... args ) const
   {
      ...
      // fmt 10.x+: requires lvalues and explicit enum formatting
      // Convert enums to underlying type for compatibility
      fmt::format_to(std::back_inserter(buf), fmt::runtime(format_str), 
                     detail::format_arg(args)...);
